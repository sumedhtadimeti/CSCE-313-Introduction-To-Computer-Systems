...
int main(int argc, char *argv[])
{
    ...
    while ((code = getopt(argc, argv, "m:n:b:w:p:f:i:")) != -1){
    switch (code) {
        case 'm': 
            m = atoi(optarg);
            break;
    ...
    if (pid == 0){
        ...
        //if (m==0){ // IF M FLAG IS TRIGGERED
            execl ("server", "server","-m",m2.c_str(), "-i",ival.c_str(), (char *)NULL);
      //  }
        //if(m!=0){ // IF M FLAG IS NOT TRIGGERED
          //  execl ("server", "server", "-i",ival.c_str(), (char *)NULL);
    //    }
    }
    ...
    for (int i=0;i<w;i++){
        wchans[i]= create_new_channel(chan, ival, m); // creates the channel
    }
