...
private:
    ...
    //for thread-saftey
    mutex m;
    condition_variable data_available; // for wait by the pop() and sugnaled by push()
    condition_variable slot_available; // waited on by push(), signaled after pop()
...
public:
	...
	void push(char* data, int len){
		...
		vector<char> d (data, data+len); // this is called a range vector
		//1. Wait until there is room in the queue (i.e., queue lengh is less than cap)
		unique_lock<mutex> l(m);
		slot_available.wait(l, [this]{return q.size()<cap;});
		//2. Then push the vector at the end of the queue, watchout for race condition
		q.push(d);
		l.unlock(); // option unlock
	...
	int pop(char* buf, int bufcap){
		...
		assert(d.size()<=bufcap); // if false, it will crash the program
